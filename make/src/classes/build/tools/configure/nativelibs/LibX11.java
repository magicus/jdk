...
public class LibX11 extends LibraryInfo {
    public LibX11(String flagName, String libName, String includeTestName, boolean available, boolean required) {
        super("x", null, "X11/Xlib.h", getX11Available(), getX11Required());
    }

    /*
    here is a lot of mess
    copy the original autoconf code for setting up X,
    but also add our special path lookup.
     */

    public static void setupX11() {
        // need alias "x" for "x11"
        LibraryInfo x11 = LibraryInfo.register("x11", null, "cups/cups.h", getX11Available(), getX11Required());

        x11.checkOptions();

    }

    private static boolean getX11Required() {
        return true;
    }

    private static boolean getX11Available() {
        return true;
    }

    @Override
    protected LibraryPaths[] getWellKnownPaths() {
        return super.getWellKnownPaths();
    }
...
[
  if test "x$NEEDS_LIB_X11" = xfalse; then
    ...
    if test "x$COMPILE_TYPE" = xcross || (test "x$SYSROOT" != "x" && test "x$x_libraries_orig" = xNONE); then
      ...
      Note! Should be * instead of + above! but causes problems with java comment...
    fi
