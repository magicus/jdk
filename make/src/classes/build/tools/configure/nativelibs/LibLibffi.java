...
import build.tools.configure.core.AC;
import build.tools.configure.core.LibraryFlags;
import build.tools.configure.core.LibraryInfo;
import build.tools.configure.core.Option;

public class LibLibffi {

    private Option bundleOpt;
    private LibraryInfo libffi;

    public void setupLibffi() {
        libffi = LibraryInfo.register("libffi", "ffi", "ffi.h", getLibffiAvailable(), getLibffiRequired());

        libffi.checkOptions();
        libffi.tryLinkLibrary(null);

    }

    public void setupLibffiBundling() {
        this.bundleOpt = Option.registerEnable("libffi-bundling");
        if (bundleOpt.isEnabled()) {
            String libDir = libffi.getFlags().getLibPath();
            if (libDir == null) {
                AC.fatal("Bundling of " + libffi.getFlagName() + " requested, but library path cannot be derived");
            }

            String libPath = LibraryInfo.tryLocateLibrary(libDir, libffi.getLibraryName());
            // FIXME: register libPath for bundling
        }
    }

    protected void tryLinkLibrary(LibraryFlags flags) {
        String testInclude = "ffi.h";

        String includeLine = "#include <" + testInclude + ">";

        String libCallLine = "ffi_call(NULL, NULL, NULL, NULL)";

        String program = includeLine + "\n" +
                "int main() {\n" +
                "    " + libCallLine + ";\n" +
                "    return 0;\n" +
                "}";

        String linkCommandLine = "ac_link='$CC -o conftest.exe $CFLAGS $CPPFLAGS $LDFLAGS conftest.c $LIBS";
    }

    private void additionalChecksWithLib(LibraryFlags flags) {
        String testProgram = """
                      #include <ffi.h>
        ...
        boolean success = AC.tryCompile(testProgram, flags);
        if (!success) {
            flags = flags.appendCflags("-DFFI_GO_CLOSURES=0");
        }
    ...
    private static boolean getLibffiRequired() {
        return true;
    }

    private static boolean getLibffiAvailable() {
        return true;
    }
    /*
